...
async def get_current_user(db: Session = Depends(get_db), token: str = Depends(oauth2_scheme)):
    ...
    try:
        ...
        raise credentials_exception
    user = db.query(Users).where(Users.name == token_data.name).first()
    if user is None:
        raise credentials_exception
    return user
...
async def login_for_access_token(db: Session = Depends(get_db), form_data: OAuth2PasswordRequestForm = Depends()):
    user = db.query(Users).where(Users.name == form_data.name, Users.status == True).first()
    if user:
    ...
    db.query(Users).filter(Users.name == form_data.name).update({
